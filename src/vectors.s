...
_swi:
    ...
    */
    ...
    mov r0, sp              /* the trap frame is done, so save a pointer to it */

    and r1, sp, #4          /* test stack alignment */
    sub sp, sp, r1          /* and adjust the stack to be 8 byte aligned */
    stmfd sp!, {r1, lr}     /* store alignment and lr (we store lr too just to
                               keep the stack aligned) */

    bl swi                  /* call it */

    ldmfd sp!, {r0, lr}     /* pop stack alignment (and the padding, composed of lr) */
